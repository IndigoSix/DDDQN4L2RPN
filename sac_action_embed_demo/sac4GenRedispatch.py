...
class SoftAC(object):
    def __init__(self, state_dims, state_embed_dim, action_dim, state_embed_hiddens, hiddens,
    ...
    def _build(self):
        ...
        self.a_hat_pl = tf.placeholder(tf.float32, shape=(None, self.action_dim))
        ...
        with tf.variable_scope('q1'):
            q1 = tf.squeeze(mlp1(tf.concat([self.states, self.a_hat_pl], axis=-1), self.hiddens + [1],
        ...
        with tf.variable_scope('q2'):
            q2 = tf.squeeze(mlp1(tf.concat([self.states, self.a_hat_pl], axis=-1), self.hiddens + [1],
                                 activation=tf.nn.relu), axis=1)
    ...
    def train(self, step, state, a_hat, next_state, reward, done):
        feeds = {self.states_pl: state, self.a_hat_pl: a_hat,
